...
use metalos_disk::{DiskFile, MEGABYTE};
 ...
 */
pub fn lazy_wipe(mut disk_file: DiskFile) -> Result<DiskFile> {
    ...
    for _ in 0..64 {
        file.write_all(&empty_mb_buf)
            .context("Failed to write zeroes to beginning of disk")?
    }

    // Wipe last 64M
    file.seek(SeekFrom::End((MEGABYTE as i64) * -64))?;
    for _ in 0..64 {
        file.write_all(&empty_mb_buf)
...
pub mod tests {
    ...
    fn check_empty(disk_file: &mut DiskFile, start: SeekFrom, megabytes: u64) -> Result<()> {
        let empty_buf = [0; 1024 * 1024];
        let mut out_buf = [0; 1024 * 1024];
        let mut file = &disk_file.0;
        file.seek(start)?;
        for _ in 0..megabytes {
            file.read_exact(&mut out_buf)
    ...
    fn write_random(disk_file: &mut DiskFile, start: SeekFrom, megabytes: u64) -> Result<()> {
        let mut file = &disk_file.0;
    ...
    fn test_lazy_wipe() -> Result<()> {
        ...
        let mut disk_file = disk.open_as_file(true)?;
