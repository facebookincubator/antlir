...
def _do_not_use_directly_get_cfg(name, default = None):
    # TODO: Uncomment this after we remove our stuff from .buckconfig.in
...
def _get_optional_str_cfg(name):
    return _do_not_use_directly_get_cfg(name) or None
...
#   - `buck -c`
#   - `.buckconfig`
#   - `do_not_use_repo_cfg` from `oss_shim.bzl`
#   - defaults here, in `constants.bzl` -- these have to be reasonable
#     since this is what a clean open-source install will use
#
...
REPO_CFG = struct(
    ...
    version_set_default = _get_optional_str_cfg("version_set_default"),
)
