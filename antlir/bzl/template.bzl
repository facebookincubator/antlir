...
_template = rule(
    ...
    attrs = {
        ...
        "_compile_template": attrs.default_only(
            attrs.exec_dep(
                providers = [RunInfo],
...
def template(visibility = None, **kwargs):
    _template_macro(visibility = get_visibility(visibility), **kwargs)
...
_render = rule(
    ...
    attrs = {
        ...
        "_render_template": attrs.default_only(
            attrs.exec_dep(
                providers = [RunInfo],
...
def render(*, instance: typing.Any, visibility = None, **kwargs):
    _render_macro(data_json = selects.apply(instance, json.encode), visibility = get_visibility(visibility), **kwargs)
