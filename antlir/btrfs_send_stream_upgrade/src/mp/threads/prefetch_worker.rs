...
pub struct PrefetchWorker {
    /// The name associated with the prefetch worker
    pw_name: String,
    /// The join handle to check the status of the prefetch worker
    pw_status: Option<JoinHandle<anyhow::Result<()>>>,
}

impl PrefetchWorker {
    fn prefetch_work(_context: SendStreamUpgradeContext) -> anyhow::Result<()> {
        Ok(())
    }
}
...
impl Worker for PrefetchWorker {
    fn new(name: String, context: &mut SendStreamUpgradeContext) -> anyhow::Result<Self> {
        let sync_container = match context.ssuc_sync_container {
            ...
            None => {
                anyhow::bail!("Creating new prefetch worker for context without sync container")
            }
        };
        ...
        Ok(Self {
            pw_name: name,
            pw_status: Some(thread::spawn(move || Self::prefetch_work(new_context))),
        })
    }
    fn get_status(&mut self) -> anyhow::Result<bool> {
        match self.pw_status {
            Some(ref handle) => {
                if !handle.is_finished() {
                    return Ok(true);
                }
            }
            None => anyhow::bail!("Failed to get status handle in prefetch worker"),
        }
        // The thread is done now
        // Remove the join handle and look it up
        let handle = match self.pw_status.take() {
            Some(handle) => handle,
            None => anyhow::bail!("Unexepcted None status handle in prefetch worker"),
        };
        match handle.join() {
            Ok(Ok(())) => Ok(false),
            ...
            Err(e) => anyhow::bail!("Thread {} paniced because {:?}", self.pw_name, e),
        }
    }
}
