...
pub struct CompressionWorker {
    /// The name associated with the compression worker
    cw_name: String,
    /// The join handle to check the status of the compression worker
    cw_status: Option<JoinHandle<anyhow::Result<()>>>,
}

impl CompressionWorker {
    fn compression_work(_context: SendStreamUpgradeContext) -> anyhow::Result<()> {
        Ok(())
    }
}
...
impl Worker for CompressionWorker {
    fn new(name: String, context: &mut SendStreamUpgradeContext) -> anyhow::Result<Self> {
        let sync_container = match context.ssuc_sync_container {
            ...
            None => {
                anyhow::bail!("Creating new compression worker for context without sync container")
            }
        };
        ...
        Ok(Self {
            cw_name: name,
            cw_status: Some(thread::spawn(move || Self::compression_work(new_context))),
        })
    }
    fn get_status(&mut self) -> anyhow::Result<bool> {
        match self.cw_status {
            Some(ref handle) => {
                if !handle.is_finished() {
                    return Ok(true);
                }
            }
            None => anyhow::bail!("Failed to get status handle in compression worker"),
        }
        // The thread is done now
        // Remove the join handle and look it up
        let handle = match self.cw_status.take() {
            Some(handle) => handle,
            None => anyhow::bail!("Unexepcted None status handle in compression worker"),
        };
        match handle.join() {
            Ok(Ok(())) => Ok(false),
            ...
            Err(e) => anyhow::bail!("Thread {} paniced because {:?}", self.cw_name, e),
        }
    }
}
