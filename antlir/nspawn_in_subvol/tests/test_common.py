...
class CommonTestCase(unittest.TestCase):
    ...
    def test_cgroup2_mountpoint_usual(self):
        with patch(
            ...
            mock_open(
                read_data=b"""cgroup2 /sys/fs/cgroup cgroup2 rw,nosuid,nodev,noexec,relatime 0 0\n/dev/mapper/ssd-ssdstripe / btrfs rw,relatime,compress=zstd:3,ssd,space_cache,subvolid=5,subvol=/ 0 0\n"""
            ),
        ):
    ...
    def test_cgroup2_mountpoint_unified(self):
        with patch(
            "antlir.nspawn_in_subvol.common.open",
            mock_open(
                read_data=b"""cgroup2 /sys/fs/cgroup/unified cgroup2 rw,nosuid,nodev,noexec,relatime 0 0\n/dev/mapper/ssd-ssdstripe / btrfs rw,relatime,compress=zstd:3,ssd,space_cache,subvolid=5,subvol=/ 0 0\n"""
            ),
        ):
            self.assertEqual(
