...
class S3StorageTestCase(StorageBaseTestCase):
    ...
    def test_write_and_read_back(self):
        ...
        writes = self.check_storage_impl(self.storage)
        for contents, sid in writes:
            self.s3.upload_fileobj.called_with(
                contents, "antlir-test", self.storage._object_key(sid)
            )
