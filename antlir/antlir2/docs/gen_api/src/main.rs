...
fn main() -> Result<()> {
    ...
    let feature_defs = buck::starlark_doc(
        "fbcode//antlir/antlir2/bzl/feature:defs.bzl"
            .parse()
            .expect("valid label"),
    )
    ...
    let feature_defs = tera.render("templates/features.mdx", &context)?;

    if let Some(out) = args.out {
        std::fs::write(out.join("features.mdx"), feature_defs)?;
    } else {
