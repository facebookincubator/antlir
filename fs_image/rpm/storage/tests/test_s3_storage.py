    ...
    def mock(*args, **kwargs):
        with temp_dir() as td, Path.resource(
            ...
            with unittest.mock.patch.object(
                S3Storage, "_base_cmd", return_value=[mock_s3_cli_path]
...
class S3StorageTestCase(CLIObjectStorageBaseTestCase):
    def setUp(self):
        ...
        self.storage = Storage.make(key="test", kind="s3", timeout_seconds=3)
    ...
    def test_error_cleanup(self):
        ...
        def _mock_path_for_storage_id(sid):
            return os.path.join("/not/a/valid/path", sid)
    ...
    def test_base_cmd(self):
        self.assertEqual(
            Storage.make(key="test", kind="s3", timeout_seconds=123)._base_cmd(
                "--cat", "meow"
            ),
            [
                "aws",
    ...
    def test_path_for_storage_id(self):
        ...
        blob_sid = "123"
        with unittest.mock.patch.object(
            S3Storage, "AWS_S3_BUCKET", s3_bucket_name
        ):
            self.assertEqual(
                Storage.make(key="test", kind="s3")._path_for_storage_id(
                    blob_sid
                ),
                os.path.join(s3_bucket_name, "flat", blob_sid),
            )
