...
class TestRenameShadowedInternals(unittest.TestCase):
    ...
    def test_get_shadowed_original(self):
        ...
        self.assertEqual(
            self._shadow / 'borsch',
            self._get_shadowed_original(b'/./borsch'),
        )
        ...
        self.assertEqual(
            self._shadow + b'/',
            self._get_shadowed_original(b'/'),
        )
        # The only error case we can reasonably check
        self.assertEqual(
            None,
            self._get_shadowed_original(b'/dir_that_does_not_exist/foo'),
        )
    ...
    def test_get_shadowed_rename_dest(self):
        with temp_dir() as td:
            ...
            self.assertEqual(None, self._get_shadowed_rename_dest(
