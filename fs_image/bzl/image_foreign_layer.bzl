...
filesystem operation as part of `fs_image/compiler` core.  This is where the
"foreign layer" abstraction comes in.

A foreign layer runs a command inside the snapshot of a parent image, and
captures the resulting filesystem as its output.  It is the `fs_image`
...
`rpmbuild` works this way.  Follow that same pattern to get your files:
  ...
  - The moment you need to use such outputs as inputs to a regular Buck
    macro, ping `fs_image` devs, and we'll provide an `image.source`
    analog that copies files out of the image via `find_built_subvol`.

## Rules of `image_foreign_layer` usage:

  - Always get a code / design review from an `fs_image` maintainer.
  - Do not use in `TARGETS` / `BUCK` files directly.  Instead, define a
    `.bzl` macro named `image_<intended_action>_layer`.
  - Place your macro in `fs_image/bzl/foreign/<intended_action>`.
  - Do not change any core `fs_image` code when adding a foreign layer.  If
    your foreign layer requires changes outside of `fs_image/bzl/foreign`,
    discuss them with `fs_image` maintainers first.
  - Tests are mandatory, see `fs_image/bzl/foreign/rpmbuild` for a good
    example.
...
# Deliberate limitations of the `image_foreign_layer` implementation
  ...
  - No network access. This is the gateway to non-deterministic hell.
    If you're sure your use-case is "safe", talk to `fs_image` maintainers
